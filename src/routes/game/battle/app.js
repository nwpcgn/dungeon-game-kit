...
*/
	...
	attack(target) {
		...
		if (gamble > 0.9) {
			...
			Display.updateCombatLog(
				`IT WAS A CRITICAL HIT ${this.name}!!!! ${critHit} points of damage were dealt`
			)
			Display.attack(true)
		...
		Display.attack()
		Display.updateCombatLog(
			`${this.name} attacks ${target.name} dealing ${damage} points of damage`
		)
	...
	defend() {
		this.def = this.def * 2
		Display.updateCombatLog(
			"With the power of the gods, you gather resilience to withstand your mighty foe's attack. Defence increased by 100% for the following opponent's attack"
		)
		return this.def
	}
	...
	heal() {
		if (this.mana < 30) {
			if (this.name === 'Abby') {
				Display.updateCombatLog(
					"I don't have enough mp, therefore I will attack you Ybba"
				)
				this.attack(ch2)
				return ch2.hp
			}
			Display.updateCombatLog(
				`${this.name}, you don't have enough MP for that move`
			)
		} else {
			let heal = 30
			this.hp += heal
...
class CombatSystem {
    ...
    */
	...
	beginCombat() {
		...
		this.increaseTurn()
		Display.updateCombatLog(
			'Combat has begun. Read instructions below (Scroll down)'
		)
		return this.status
	}
	...
	oneTurnEffectsReset() {
		...
		Display.updateCombatLog(
			'All one turn effects buffed stats have been restored to origin status'
		)
		this.oneTurnEffects = false
	}
}
...
class Display {
	static updateCombatStats() {
		for (let i = 0; i < document.querySelectorAll('.ch').length; i++) {
			...
			charContainers[i].children[1].innerHTML =
				`${i === 0 ? ch1.name : ch2.name + ' (you)'}`
			charContainers[i].children[2].innerHTML =
				`${i === 0 ? ch1.hp : ch2.hp}/${i === 0 ? ch1.hpOrigin : ch2.hpOrigin}HP`
			charContainers[i].children[3].innerHTML =
				`${i === 0 ? ch1.mana : ch2.mana}/${i === 0 ? ch1.manaOrigin : ch2.manaOrigin}MP`
		}
