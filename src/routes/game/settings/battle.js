class Game {
	...
	initializeGame() {
		this.player = new Player('Überlebender', 100, 10)
		this.generateLevels()
...
class Player {
	...
	takeDamage(amount) {
		...
		console.log(
			`${this.name} nimmt ${amount} Schaden. Verbleibende Gesundheit: ${this.health}`
		)
		if (this.health <= 0) {
	...
	attack(enemy) {
		const damage = Math.floor(this.attackPower * (0.8 + Math.random() * 0.4))
		console.log(
...
class Level {
	...
	generateContent() {
		const enemyTypes = ['Wächter-Roboter', 'Mutant', 'Drohne']
...
class Enemy {
	constructor(name, health, attack) {
		...
		this.health = health
		this.attack = attack
		this.experienceValue = Math.floor(health / 2)
	}
